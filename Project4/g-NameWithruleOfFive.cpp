...
public:
   ...
   //indicate that the parameter N is an rvalue reference to a Name object
   /*
   Rvalues are temporary and will not be used after the move operation. Thus, their resources can be safely "stolen" (transferred).
   Move constructors and assignment operators take an rvalue reference as a parameter, which ensures that only temporary objects can be moved.
   */
   Name(Name&& N) {
